<?php
// filepath: /Applications/XAMPP/xamppfiles/htdocs/dvlaregister/src/views/admin/download-qr.php
session_start();
...
try {
    // Admin can access any vehicle, so we only need to verify the admin user exists
    $userStmt = $pdo->prepare("SELECT * FROM users WHERE id = :user_id AND role = 'admin'");
    $userStmt->execute(['user_id' => $userId]);
    $user = $userStmt->fetch();
    
    if (!$user) {
        die("Admin user not found or unauthorized");
    }
    
    // Get vehicle and owner details - NO PERMISSION CHECK NEEDED FOR ADMIN
    $stmt = $pdo->prepare("
    ...
    $pdf->Cell(0, 5, 'Generated by Admin: ' . htmlspecialchars($user['full_name']) . ' on ' . date('d F Y'), 0, 1);
